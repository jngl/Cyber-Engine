...
namespace math {
	...
	class Vec3
	{
	  public:
		...
		Vec3<T>(const Vec2<T> &source, const T Z){
		  x = source.x;
		  y = source.y;
		  z = Z;
		}
		...
		inline Vec3<T> operator + (const Vec3<T> &right){
		  return Vec3(right.x + x, right.y + y, right.z + z);
		}
	 
		inline Vec3<T> operator - (const Vec3<T> &right){
		  return Vec3<T>(-right.x + x, -right.y + y, -right.z + z);
		}
	 
		inline Vec3<T> operator * (const T scale){
		  return Vec3<T>(x*scale, y*scale, z*scale);
		}
	 
		inline Vec3<T>  operator / (const T scale){
		  return scale ? Vec3<T>(x/scale, y/scale, z/scale): Vec3<T>(0, 0, 0);
		}
		...
		inline void set(const Vec3<T> & t)
		{
		  x = t.x;
		  y = t.y;
		  z = t.z;
		...
		inline const T getDotProduct(const Vec3<T> &t) const
		{
		  return x*t.x + y*t.y + z*t.z;
		}

		inline Vec3<T> operator ^(const Vec3<T> &t)
		{
		...
		inline Vec3<T> &crossProduct(const Vec3<T> &t1, const Vec3<T> &t2)
		{
		...
		inline const T getDistance(const Vec3<T> &v2)  const 
		{
		...
		inline const T getAngle(const Vec3<T> &v2)  const 
		{             
		...
		inline Vec3<T> &clamp(T min, T max)
		{
		...
		friend std::istream & operator >> (std::istream & in, Vec3<T> &right){
